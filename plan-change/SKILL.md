...
4. If multiple solutions exist, compare at most 3 candidate plans.
5. Produce a plan another agent can implement directly.
6. Update `.ace/ACE.md` with decisions, acceptance criteria, and plan summary (phase = Implementation ready).
...
3. If multiple approaches exist, define up to 3 candidate plans with assumptions.
4. Evaluate candidates using a small rubric and select one; document the rationale.
5. Break work into incremental, testable steps.
6. Specify tests: unit and integration/end-to-end as relevant.
7. Call out migration/rollout considerations (flags, config, data migrations).
8. Write the plan to `.ace/plan.md` and update `.ace/ACE.md`.

## Output format
Provide `.ace/plan.md` with:
1. **Planned Behavior**: plain-language description plus edge cases and expected handling.
2. **Candidate Plans (if needed, max 3)**: short descriptions, assumptions, and tradeoffs.
3. **Evaluation Matrix**: criteria + 1-5 scores + brief notes per candidate.
4. **Decision**: selected plan + rationale + why others were rejected.
5. **Change Set Overview**: files to change and what will change.
6. **Implementation Steps**: ordered, numbered steps that are small and testable.
7. **Tests**: unit test cases (given/when/then) and integration scenarios with expected outcomes.
8. **Rollout / Risk Notes**: potential failure modes, monitoring/metrics, flags/config strategy.
